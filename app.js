...
function renderDashboard(){
  ...
  stats.innerHTML=`
    ...
    <div class="card holiday-card"><small>Jours fériés</small><b>${totals.holiday.toFixed(1)}h</b></div>`;

  let first=new Date(y,m,1),n=new Date(y,m+1,0).getDate(),off=(first.getDay()+6)%7,h='<div class="legend"><span><i class="dot day-dot"></i> Jour</span><span><i class="dot night-dot"></i> Nuit</span><span><i class="dot sunday-dot"></i> Dimanche</span><span><i class="dot holiday-dot"></i> Férié</span></div><div class="month-grid">';
  ...
  for(let d=1;d<=n;d++){
    ...
    h+=`<div class="day ${dayClass}"><b>${d}</b>${ss.map(x=>{
      ...
      return `<div class="mission ${classForShift(x)}">${esc(x.sites?.name)}<br>${x.start_time.slice(0,5)}-${x.end_time.slice(0,5)}${profile.role==="admin"?`<br>${esc(x.profiles?.full_name)}`:""}<div class="hours-mini">J ${sum.day.toFixed(1)}h · N ${sum.night.toFixed(1)}h</div></div>`;
    }).join("")}</div>`;
  }
  monthPlanning.innerHTML=h+"</div>";
}
function renderAgents(){agentsList.innerHTML=agents.map(a=>`<div class="item"><div><b>${esc(a.full_name)}</b><br><small>${esc(a.phone||"")} · ${esc((a.qualifications||[]).join(", "))}</small></div><button onclick="openAgent('${a.id}')">Modifier</button></div>`).join("")||"<p>Aucun agent.</p>"}
...
function renderShifts(){shiftsList.innerHTML=shifts.map(s=>{const x=summaryForShift(s);return `<div class="item"><div><b>${s.shift_date} — ${esc(s.sites?.name)}</b><br><small>${s.start_time.slice(0,5)}-${s.end_time.slice(0,5)} · ${esc(s.profiles?.full_name||profile.full_name)} · Jour ${x.day.toFixed(1)}h · Nuit ${x.night.toFixed(1)}h${x.sunday?` · Dimanche ${x.sunday.toFixed(1)}h`:""}${x.holiday?` · Férié ${x.holiday.toFixed(1)}h`:""}</small></div>${profile.role==="admin"?`<button onclick="openShift('${s.id}')">Modifier</button>`:""}</div>`}).join("")||"<p>Aucune mission.</p>"}
function renderLeaves(){c.from("leave_requests").select("*,profiles!leave_requests_agent_id_fkey(full_name)").order("created_at",{ascending:false}).then(r=>{let rows=(r.data||[]).filter(x=>profile.role==="admin"||x.agent_id===profile.id);leavesList.innerHTML=rows.map(x=>`<div class="item leave ${x.status}"><div><b>${esc(x.profiles?.full_name||profile.full_name)} — ${esc(x.type)}</b><br><small>${x.start_date} → ${x.end_date}</small></div><div>${x.status}${profile.role==="admin"&&x.status==="pending"?`<br><button onclick="decide('${x.id}','approved')">Accepter</button> <button onclick="decide('${x.id}','rejected')">Refuser</button>`:""}</div></div>`).join("")||"<p>Aucune demande.</p>"})}
...
agentForm.onsubmit=async e=>{e.preventDefault();let p={full_name:agentName.value,phone:agentPhone.value,card_number:agentCard.value,card_expiry:agentExpiry.value||null,qualifications:agentQualifs.value.split(",").map(x=>x.trim()).filter(Boolean)};if(!agentId.value)return alert("Pour créer un nouveau compte automatiquement, on branche l'invitation email à l'étape suivante.");let r=await c.from("profiles").update(p).eq("id",agentId.value);if(r.error)return alert(r.error.message);agentDialog.close();await refresh()}
...
(async()=>{let r=await c.auth.getSession();if(r.data.session){session=r.data.session;await boot()}})();  const p=splitHours(s.start_time,s.end_time);
  return {
    ...p,
    sunday:isSunday(s.shift_date)?p.total:0,
    holiday:isHoliday(s.shift_date)?p.total:0
